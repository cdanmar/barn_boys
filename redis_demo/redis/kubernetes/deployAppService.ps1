...
try {
    Write-Verbose "Deploying App Deployment" -Verbose

    # Get K8s manifest file path
    $AKS_DEPLOY_APP_FILE = (Join-Path $SCRIPT_DIRECTORY "redis_deployment.yaml")
    ...
    Write-Verbose "Successfully Deployed App Deployment" -Verbose
    ...
    # Get K8s manifest file path
    $AKS_DEPLOY_APP_FILE = (Join-Path $SCRIPT_DIRECTORY "redis_service.yaml")
    Write-Verbose "AKS_DEPLOY_APP_FILE: $AKS_DEPLOY_APP_FILE" -Verbose
